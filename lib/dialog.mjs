...
export function predictableLines(classroom) {
  ...
  // 寄り添いの一言は、直後に来やすい質問と組みで作り置きする
  // (別々に合成しても、読み上げは1つの文字列なのでキャッシュが効かない)
  const empathy = Object.values(empathyLines(classroom));
  const followUps = lines.filter(
    (l) => l.includes("ご希望はいかがでしょうか") || l.includes("どちらをご希望でしょうか")
  );
  for (const e of empathy) {
    lines.push(`${e} ${FIXED_LINES.askGrade}`);
    lines.push(`${e} ${FIXED_LINES.askGradeAgain}`);
    for (const f of followUps) lines.push(`${e} ${f}`);
  }
...
/**
 * 寄り添いの一言。迷い・不安がにじむ発話には、質問より先にこれを返す。
 * 「迷ってるんだよね」に即「何年生ですか」と返すのは事務的すぎる。
 * 事実(体験無料など)は notes に書かれている場合だけ口にする。それ以外は
 * 事実を作らない言い回しに留める(§3.4 憶測で答えない)。
 */
export function empathyLines(classroom) {
  const lines = {
    hesitation:
      "ご検討中でいらっしゃいますね。よろしければ、ご案内だけでもさせていただきます。",
    concern:
      "ご安心ください。ご不安な点は、先生からも折り返しの際に詳しくご案内いたします。",
  };
  if (classroom.notes.some((n) => /体験/.test(n) && /無料/.test(n))) {
    lines.hesitation =
      "ご検討中でいらっしゃいますね。体験は無料ですので、まずはお気軽にお試しいただけます。";
  }
  return lines;
}
...
export function createSession({ classroom, callerNumber = "" }) {
  return {
    ...
    needsConsult: false,
    /** 寄り添いの一言は、気持ちの種類ごとに1回だけ。繰り返すとわざとらしい */
    empathizedKinds: [],
    name: "",
    phone: "",
    lastPrompt: "",
...
export async function step(session, utterance) {
  ...
  let result = await route(session, utterance, read);

  // 迷い・不安がにじんでいたら、本題の前に一言添える(気持ちの種類ごとに1回)
  if (
    (read.feeling === "hesitation" || read.feeling === "concern") &&
    !session.empathizedKinds.includes(read.feeling) &&
    (read.intent === "trial" || session.intent === "trial") &&
    !result.done
  ) {
    session.empathizedKinds.push(read.feeling);
    const line = empathyLines(session.classroom)[read.feeling];
    result = { ...result, say: `${line} ${result.say}` };
  }
