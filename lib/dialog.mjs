...
import { composeReply, HANDOFF_LINE, interpret, rephrase } from "./claude.mjs";
...
export function createSession({ classroom, callerNumber = "" }) {
  return {
    ...
    needsConsult: false,
    /** ごきょうだいの学年。2人目以降は日時調整が要るので先生に渡す */
    siblingGrades: [],
    name: "",
    phone: "",
    lastPrompt: "",
 ...
 */
function goConsult(session, requestedTime, lead = FIXED_LINES.consultTime) {
  session.needsConsult = true;
  if (requestedTime) session.requestedTime = String(requestedTime).trim();
  return goContact(session, lead);
}
...
export async function step(session, utterance) {
  ...
  // 言い方だけ会話の流れに合わせる。内容(何を言うか)は route が決めたまま。
  // 定型で足りる場面まで作り直すと、遅くなるうえに揺れるので絞る
  if (shouldRephrase(session, utterance, read, result)) {
    const history = session.turns
      .slice(-3)
      .map((t) => `保護者: ${t.utterance}\n教室: ${t.reply || "(これから)"}`)
      .join("\n");
    // 迷い・不安を見せているときだけ、先生の案内事項を安心材料として渡す
    const facts =
      read.feeling === "hesitation" || read.feeling === "concern"
        ? session.classroom.notes
        : [];
    const natural = await rephrase({ text: result.say, history, facts });
    if (natural) result.say = natural;
  }
 ...
 * 言い方を作り直すべき場面か。
 *
 * 「はい」「水曜日で」のような素直な返事に定型で返すのは自然なので、
 * そのまま使う(事前合成が効いて速い)。作り直すのは、相手が質問した・
 * 気持ちを見せた・こみ入ったことを言った場合に限る。
 */
function shouldRephrase(session, utterance, read, result) {
  if (result.done) return false; // 終話の挨拶は定型でよい
  if (read.feeling === "hesitation" || read.feeling === "concern") return true;
  if (read.questionTopic !== "none") return true;
  if (session.needsConsult || session.siblingGrades.length > 0) return true;
  if (/[?？]/.test(utterance)) return true;
  // 長い発話は事情を含んでいることが多い
  return utterance.replace(/[、。\s]/g, "").length >= 18;
}
...
async function route(session, utterance, read) {
  ...
  // ごきょうだいの同時申込。2人分の日時を電話で組むのは無理があるので、
  // 両方を記録したうえで日時は先生に調整してもらう(仮予約自体は取る)。
  //
  // LLMの siblingGrade は取りこぼすことがあり、取りこぼすと「学年の言い直し」と
  // 誤認して1人目が消える。「〜の子もいる」等の字面を必ず併用する。
  const mentionsSibling =
    /(も(いる|います|一緒|いっしょ|お願い|受け)|下の子|上の子|きょうだい|兄弟|姉妹|ふたり|二人|2人)/.test(
      utterance
    );
  const sibling =
    normalizeGrade(read.siblingGrade) ??
    (mentionsSibling ? normalizeGrade(utterance) : null);
  if (
    session.intent === "trial" &&
    sibling != null &&
    session.grade != null &&
    sibling !== session.grade &&
    !session.siblingGrades.includes(sibling)
  ) {
    session.siblingGrades.push(sibling);
    session.unclearCount = 0;
    const both = `${gradeLabel(session.grade)}と${gradeLabel(sibling)}`;
    return goConsult(
      session,
      `${both}のお二人`,
      `${both}のお二人ですね。ご一緒に体験いただけるよう、先生が日程を調整いたします。`
    );
  }
...
export function summarize(session) {
  return {
    ...
    siblingGrades: session.siblingGrades.map(gradeLabel),
